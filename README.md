...
Hostname routing and zone controls live in Cloudflare, not in this repo: keep `alexnava.me` active on Pages project `alexnava-me`, redirect `www.alexnava.me` to the apex, and keep `*.pages.dev` traffic non-indexable/access-controlled. The `_headers` file sends `X-Robots-Tag: noindex` for both `https://:project.pages.dev/*` and `https://:version.:project.pages.dev/*`; re-confirm the Cloudflare project, DNS, certificate, WAF, and CAA state before any deploy or DNS/security setting change.

...
test("the loading ritual is decorative, self-contained, timed, and motion-safe", async () => {
  ...
  assert.match(ritual, /class="loading-ritual__motto">Effective challenge\.</);
...
test("hero and About copy remain clear, grounded, and free of scramble hooks", async () => {
  ...
  assert.equal(
    heroText,
    "Regulated analytics Governance. Analytics. Effective challenge. Traceable conclusions for risk, compliance, and audit.",
  );
...
test("career metadata stays consistent and scene discovery uses inert metadata", async () => {
  const html = await readIndexHtml();
  const description = "Governance, analytics, and effective challenge for audit-sensitive work.";
